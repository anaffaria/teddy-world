...
public class CustomerConstraints implements IStrategy {
    ...
    public String applyBusinessRule(DomainEntity domainEntity) {
        if (domainEntity instanceof Customer) {
            ...
            if(!matcher.matches()) stringBuilder.append("A Senha deve conter numeros, caracteres especiais e 8 digitos");
            if(customer.getPassword() != customer.getPasswordConfirm()) stringBuilder.append("As senhas não conferem");
        }
        return "";
    }
}
